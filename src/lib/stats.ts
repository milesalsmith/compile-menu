...
export function dietFilter(diet: string): MenuItem[] {
  if (diet === "vegan") return MENU.filter((p) => p.vegan);
  if (diet === "vegetarian") return MENU.filter((p) => p.vegetarian);
  return MENU;
}
...
export function statsOf(universe: MenuItem[]): Stats {
